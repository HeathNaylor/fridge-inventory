...
class Fridge implements TemperatureRegulatorInterface
{
    /**
     * @var array
     */
    ...
    public function addItem(ItemInterface $item)
    {
        switch($item->getType()) {
            case Item::TYPE_MEAT:
                $item->setExpirationDate($item->getExpirationDate()->modify("+2 weeks"));
                break;
            case Item::TYPE_PRODUCE:
                $item->setExpirationDate($item->getExpirationDate()->modify("+3 weeks"));
                break;
            case Item::TYPE_DAIRY:
                $item->setExpirationDate($item->getExpirationDate()->modify("+4 weeks"));
                break;
            default:
